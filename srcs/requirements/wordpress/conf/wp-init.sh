...
end_time=$((start_time + 30)) # set the end time to 30 seconds after the start time
...
if [ $(date +%s) -ge $end_time ]; then # check if the current time is greater than or equal to the end time
    echo "[========MARIADB IS NOT RESPONDING AFTER 30 SECONDS! CHECK CONNECTION SETTINGS!========]"
fi
...
wp admin create --dbname=$MYSQL_DB --dbuser=$MYSQL_USER --dbpass=$MYSQL_PASSWORD --path=/var/www/html --allow-root
